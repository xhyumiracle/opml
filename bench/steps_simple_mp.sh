...
export BASEDIR=/tmp/cannon_stats_mnist_test
...
# mlvm/mlvm --outputBenchmark --outputMode=2 --basedir=$BASEDIR --program="$PROGRAM_PATH" --modelName="$MODEL_NAME" --model="$MODEL_PATH" --data="$DATA_PATH" --target=0 --nodeID 0
/usr/bin/time -v mlvm/mlvm --outputBenchmark --outputMode=2 --basedir=$BASEDIR --program="$PROGRAM_PATH" --modelName="$MODEL_NAME" --model="$MODEL_PATH" --data="$DATA_PATH" --target=0 --nodeID 0 2>&1 | tee -a $LOGDIR/$MODEL_NAME/node_0.log
